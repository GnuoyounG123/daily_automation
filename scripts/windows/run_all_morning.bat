...
:: Create log
if not exist "%MASTER_DIR%runtime_local\logs" mkdir "%MASTER_DIR%runtime_local\logs"
echo %date% %time% - Morning automation started >> "%MASTER_DIR%runtime_local\logs\morning_master.log"

echo [TASK 1/3] Academic Briefing (Master)
echo ----------------------------------------
cd /d "%MASTER_DIR%"
python "daily_assistant.py" >> "%MASTER_DIR%runtime_local\logs\morning_master.log" 2>&1
...
echo ----------------------------------------
cd /d "%MASTER_DIR%"
python "schedule_manager.py" >> "%MASTER_DIR%runtime_local\logs\morning_master.log" 2>&1
