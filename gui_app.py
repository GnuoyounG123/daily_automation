...
class DailyAutomationApp:
    ...
    def _encode_pwd(value: str) -> str:
        return PasswordCrypto(Path(__file__).parent).encrypt(value)
    ...
    def refresh_home_status(self):
        ...
        self.status_labels['email_status'].config(text="已启用" if email.get('enabled') else "未启用")
    ...
    def load_reminders(self):
        ...
        for r in reminders:
            self.reminders_tree.insert('', 'end', values=(
                r.get('time', ''), r.get('title', ''), r.get('description', '')
            ))
    ...
    def create_api_keys_tab(self):
        ...
        ttk.Label(frame, text="学术API密钥配置", font=('Microsoft YaHei', 14, 'bold')).pack(pady=10)
        ttk.Label(frame, text="配置API密钥可提升访问速率和配额，部分API必须配置密钥才能使用", foreground='gray').pack(pady=5)
        form_frame = ttk.Frame(frame)
        form_frame.pack(fill='x', padx=20, pady=10)
        ...
        ttk.Button(frame, text="💾 保存API密钥配置", command=self.save_api_keys_config).pack(pady=10)
    ...
    def on_day_change(self, event):
        ...
        for course in courses:
            self.schedule_tree.insert('', 'end', values=(
                course.get('name', ''), course.get('time', ''),
                course.get('location', ''), course.get('teacher', '')
            ))
            ...
            try:
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    result = subprocess.run(
                        ...
                        cwd=str(self.app_dir), timeout=600, startupinfo=startupinfo,
                    ...
                    result = subprocess.run(
                        ...
                        cwd=str(self.app_dir), timeout=600
                    )
...
class SourceDialog:
    ...
    def ok(self):
        ...
        if name and url:
            self.result = {'name': name, 'url': url, 'source_type': self.type_var.get(), 'enabled': True}
            self.dialog.destroy()
    ...
    def ok(self):
        ...
        if name and time:
            self.result = {
                ...
                'teacher': self.entries['teacher'].get().strip(), 'note': ''
            }
            self.dialog.destroy()
