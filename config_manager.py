...
class ConfigManager:
    ...
    def add_reminder(self, time: str, title: str, description: str) -> bool:
        ...
        if 'reminders' not in config:
            config['reminders'] = []
        config['reminders'].append({'time': time, 'title': title, 'description': description})
        return self.save_config(config)
    ...
    def get_email_config(self) -> Dict:
        ...
        return config.get('email', {
            'enabled': False, 'smtp_server': 'smtp.qq.com', 'smtp_port': 587,
    ...
    def get_api_keys(self) -> Dict:
        config = self.get_config()
        return config.get('api_keys', {'semantic_scholar': '', 'openalex': '', 'core': ''})
    ...
    def check_missing_api_keys(self) -> List[Dict]:
        ...
        for key_name, info in self.API_KEY_INFO.items():
            if not keys.get(key_name, ''):
                missing.append({
                    'key_name': key_name, 'name': info['name'],
                    'required': info['required'], 'description': info['description'],
    ...
    def add_course(self, day: str, name: str, time: str, location: str,
        ...
        schedule['week_schedule'][day].append({
            'name': name, 'time': time, 'location': location, 'teacher': teacher, 'note': note
        })
